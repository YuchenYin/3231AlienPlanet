...
namespace OccaSoftware.TextureChannelPacker.Editor
{
	public class TextureChannelPackerWindow : EditorWindow
	{
		...
		{
			//Create window
			TextureChannelPackerWindow window = GetWindow<TextureChannelPackerWindow>(title: "Channel Packer");
			window.Show();
		}
		...
		private void DrawChannel(TextureData textureData, Channel channel)
		{
			...
			textureData.texture = (Texture2D)EditorGUILayout.ObjectField(textureData.texture, typeof(Texture2D), false);
			textureData.textureChannelSource = (TextureData.TextureChannelSource)EditorGUILayout.EnumPopup("Source", textureData.textureChannelSource);
			textureData.invert = EditorGUILayout.Toggle("Invert", textureData.invert);
			textureData.clearValue = EditorGUILayout.FloatField("Clear Value", textureData.clearValue);
		...
		private void DrawPreview()
		{
			if (previewOutput != null)
			{
				...
				previewMode = GUILayout.Toggle(previewMode == PreviewMode.RGB, "RGB", buttonStyle) ? PreviewMode.RGB : previewMode;
				previewMode = GUILayout.Toggle(previewMode == PreviewMode.R, "R", buttonStyle) ? PreviewMode.R : previewMode;
				previewMode = GUILayout.Toggle(previewMode == PreviewMode.G, "G", buttonStyle) ? PreviewMode.G : previewMode;
				previewMode = GUILayout.Toggle(previewMode == PreviewMode.B, "B", buttonStyle) ? PreviewMode.B : previewMode;
				previewMode = GUILayout.Toggle(previewMode == PreviewMode.A, "A", buttonStyle) ? PreviewMode.A : previewMode;
				EditorGUILayout.EndHorizontal();


				options = new GUILayoutOption[] { GUILayout.ExpandHeight(true), GUILayout.ExpandWidth(true) };
				Rect rect = EditorGUILayout.GetControlRect(false, options);

				UnityEngine.Rendering.ColorWriteMask writeMask = UnityEngine.Rendering.ColorWriteMask.All;
				...
				{
					EditorGUI.DrawPreviewTexture(rect, previewOutput, null, ScaleMode.ScaleToFit, 0, 0, writeMask);
				}
		...
		private void DrawSaveOptions()
		{
			...
			Rect buttonRect = EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight * 3);
		...
		{
			if (textureDatas[0].texture == null && textureDatas[1].texture == null && textureDatas[2].texture == null && textureDatas[3].texture == null)
				return;
		...
		{
			string filePath = EditorUtility.SaveFilePanelInProject("Save As", fileName, fileType.ToString().ToLowerInvariant(), "", directory);
